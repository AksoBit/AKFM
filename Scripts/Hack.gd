...
var Parts = preload("res://Scenes/Debris/HackBody.tscn").instantiate()
func _ready() -> void:
...
func _on_attack_area_body_entered(body: Node2D) -> void:
	...
	if body.name == "UwUGG" and can_attack and not Parried:
		...
		Parried = false
		while is_attacking:
			body.take_damage(int(speedRN / 2), 0, 0, "Hack")
			if body != null:
				FuckingVar = true
			await get_tree().create_timer(0.1).timeout 
		stop_attack()
	elif body.has_method('DESTROY') and can_attack:
		$AudioStreamPlayer2D.pitch_scale = randf_range(0.9, 1.1)
		$AudioStreamPlayer2D.play()
		$Sprite2D2/ContactPos/sparks.emitting = true
		body.DESTROY()
		...
		if body.has_method("take_damage"):
			if Parried:
				...
				if body == null:
					gg.transfer_to_text_panel("BUZZKILL", false, Color.from_rgba8(220, 213, 47, 255), Color.from_rgba8(0, 0, 0, 255))
					gg.take_overheat(5)
				elif speedRN >= 10 and body.name != "UwUGG":
					gg.transfer_to_text_panel("BUZZ-OFF", false, Color.from_rgba8(171, 229, 165, 255), Color.from_rgba8(9, 89, 0, 255))
					gg.take_overheat(int(speedRN / 8))
				Parried = false
				Parriable = true
			else:
				body.take_damage(speedRN / 4, 0, 0, "Hack")
				await get_tree().create_timer(0.01).timeout
		velocity * -1000
		$AudioStreamPlayer2D.pitch_scale = randf_range(0.9, 1.1)
		$AudioStreamPlayer2D.play()
